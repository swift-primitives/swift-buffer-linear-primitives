    ...
    Element: Copyable
{
    ...
    public var front: Element {
        base.value.storage.pointer(at: .zero).pointee
    }
    ...
    public var back: Element {
        return base.value.storage.pointer(at: base.value.header.count.subtract.saturating(.one).map(Ordinal.init)).pointee
    }
}
