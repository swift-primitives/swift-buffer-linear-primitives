...
where
    Tag == Buffer<Element>.Linear.Peek,
    ...
    Element: Copyable
{
    ...
    public var front: Element {
        base.value.storage.pointer(at: Index<Element>.Bounded<n>(.zero)!).pointee
    }
    ...
    public var back: Element {
        return base.value.storage.pointer(at: Index<Element>.Bounded<n>(base.value.header.count.subtract.saturating(.one).map(Ordinal.init))!).pointee
    }
}
